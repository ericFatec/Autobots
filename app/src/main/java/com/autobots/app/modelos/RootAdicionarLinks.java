...
import com.autobots.app.controladores.AuthController;
...
public class RootAdicionarLinks implements LinkInterface<ApiRoot>{
    ...
    public void adicionarLink(ApiRoot root) {
        ...
        Link login = WebMvcLinkBuilder
            .linkTo(WebMvcLinkBuilder.methodOn(AuthController.class).login(null))
            .withRel("login")
            .withType("POST");
        root.add(login);

        Link logout = WebMvcLinkBuilder
            .linkTo(WebMvcLinkBuilder.methodOn(AuthController.class).logout())
            .withRel("logout")
            .withType("POST");
        root.add(logout);

        Link getMe = WebMvcLinkBuilder
			.linkTo(WebMvcLinkBuilder.methodOn(UsuarioControlador.class).selecionarMe())
			.withRel("get-authenticated-user")
            .withType("GET");
		root.add(getMe);
